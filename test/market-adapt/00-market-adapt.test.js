...
contract('estimate gas - ', function (rpc_accounts) {
	...
	it('should be able to mass mint adapt tokens', async() => {
	...
	it('BUYER1 should be able to donate more than is required and take the first token', async() => {

		const tokenId = await adapt.tokenByIndex(0);

		let result = await market.buy(tokenId, { from: ac.BUYER1 , value: ether(1.1) }).should.be.fulfilled;
